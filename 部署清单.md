...
- [x] `render.yaml` - Render平台部署配置（已修复免费套餐兼容性）
- [x] `render-free.yaml` - 免费套餐专用配置（推荐）
...
#### 方案A：使用render.yaml自动部署

**免费套餐用户请使用 `render-free.yaml`：**

1. 登录 [Render](https://render.com)
2. 点击 "New +" -> "Blueprint"
3. 连接GitHub仓库
4. 选择 `render-free.yaml` 文件（不是render.yaml）
5. 设置必要的环境变量（见下方）
